...
def compute_quality_flags(df: pd.DataFrame, summary: DatasetSummary, missing_df: pd.DataFrame,) -> Dict[str, Any]:
    ...
    max_missing_share = float(missing_df["missing_share"].max()) if not missing_df.empty else 0.0
        # Дубликаты строк
    dup_share = float(df.duplicated().mean()) if len(df) > 0 else 0.0
    flags["duplicate_share"] = dup_share
    flags["too_many_duplicates"] = dup_share > 0.05

    # Константные колонки (все значения одинаковые, кроме NaN)
    constant_columns = []
