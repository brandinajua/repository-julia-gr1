...
def report(
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        # новые эвристики (если у тебя они есть в core.py)
        if "duplicate_share" in quality_flags:
            f.write(f"- Доля дубликатов строк: **{quality_flags['duplicate_share']:.2%}**\n")
        if "too_many_duplicates" in quality_flags:
        ...
        if "constant_columns" in quality_flags:
            const_cols = quality_flags["constant_columns"]
            f.write(
                f"- Константные колонки: **{', '.join(const_cols) if const_cols else 'нет'}**\n"
            )
